...
** Project management
*** Dependencies
Packages can be installed through the ~npm install~ command, the ~--save-dev~ flag is used to install dev dependencies.

The ~package.json~ file contains all project information, including dependencies and scripts, similar to ~mix.exs~.
*** Modules
